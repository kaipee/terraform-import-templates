resource "proxmox_lxc" "name" {
  ...
  mountpoint {        # An object for defining a volume to use as a container mount point. Can be specified multiple times.
    ...
    size      = "8G"  # (required) Size of the underlying volume. Must end in G, M, or K (e.g. "1G", "1024M", "1048576K"). Note that this is a read only value.
    slot      = 0     # (required) A string containing the number that identifies the mount point (i.e. the n in mp[n]).
