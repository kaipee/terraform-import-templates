resource "proxmox_vm_qemu" "name" {
  ...
  define_connection_info      = true               # Whether to let terraform define the (SSH) connection parameters for preprovisioners, see config block below.
  ...
  agent                       = 0                  # Set to 1 to enable the QEMU Guest Agent. Note, you must run the qemu-guest-agent daemon in the quest for this to have any effect.
  ...
  balloon                     = 0                  # The minimum amount of memory to allocate to the VM in Megabytes, when Automatic Memory Allocation is desired. Proxmox will enable a balloon device on the guest to manage dynamic allocation. See the docs about memory for more info.
  ...
  scsihw                      = "lsi"              # The SCSI controller to emulate. Options: lsi, lsi53c810, megasas, pvscsi, virtio-scsi-pci, virtio-scsi-single.
  pool                        = ""                 # The resource pool to which the VM will be added.
  tags                        = ""                 # Tags of the VM. This is only meta information.
  force_create                = false              # If false, and a vm of the same name, on the same node exists, terraform will attempt to reconfigure that VM with these settings. Set to true to always create a new VM (note, the name of the VM must still be unique, otherwise an error will be produced.)
  clone_wait                  = 15                 # Provider will wait clone_wait seconds after an UpdateConfig operation.
  additional_wait             = 15                 # The amount of time in seconds to wait between creating the VM and powering it up.
  preprovision                = true               # Whether to preprovision the VM. See Preprovision above for more info.
  os_type                     = ""                 # Which provisioning method to use, based on the OS type. Options: ubuntu, centos, cloud-init.
  ...
  ciuser                      = ""                 # Override the default cloud-init user for provisioning.
  cipassword                  = ""                 # Override the default cloud-init user's password. Sensitive.
  ...
  network {           # The network block is used to configure the network devices. It may be specified multiple times. The order in which the blocks are specified determines the ID for each net device. i.e. The first network block will become net0, the second will be net1 etc
    model     = ""    # (required) Network Card Model. The virtio model provides the best performance with very low CPU overhead. If your guest does not support this driver, it is usually best to use e1000
    macaddr   = ""    # Override the randomly generated MAC Address for the VM.
    bridge    = "nat" # Bridge to which the network device should be attached. The Proxmox VE standard bridge is called vmbr0.
  ...
  disk {
    type         = ""     # (required) The type of disk device to add. Options: ide, sata, scsi, virtio.
    storage      = ""     # (required) The name of the storage pool on which to store the disk.
    size         = ""     # (required) The size of the created disk, format must match the regex \d+[GMK], where G, M, and K represent Gigabytes, Megabytes, and Kilobytes respectively.
    ...
    backup       = 0      # Whether the drive should be included when making backups.
    iothread     = 0      # Whether to use iothreads for this drive. Only effective with a disk of type virtio, or scsi when the the emulated controller type (scsihw top level block argument) is virtio-scsi-single.
    ...
    discard      = ""     # Controls whether to pass discard/trim requests to the underlying storage. Only effective when the underlying storage supports thin provisioning. There are other caveots too, see the docs about disks for more info.
    ...
    media        = "disk" # The drive’s media type. Options: cdrom, disk.
